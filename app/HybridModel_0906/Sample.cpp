...
{
	imguiLabel("场景光栅化设置");	//Rasterization
	imguiSlider("体素大小", &m_cellSize, 0.1f, 1.0f, 0.01f);	//Cell Size
	imguiSlider("体素高度", &m_cellHeight, 0.1f, 1.0f, 0.01f);	//Cell Height
	...
	if (m_mesh)
	{
		...
		char text[64];
		snprintf(text, 64, "体素个数  %d x %d", gw, gh);	//Voxels
		imguiValue(text);
	}
	
	imguiSeparator();
	imguiLabel("智能体设置");	//Agent
	imguiSlider("高度", &m_agentHeight, 0.1f, 5.0f, 0.1f); //Height
	imguiSlider("半径", &m_agentRadius, 0.0f, 5.0f, 0.1f); //Radius
	imguiSlider("最大爬坡高度", &m_agentMaxClimb, 0.1f, 5.0f, 0.1f); //Max Climb
	imguiSlider("最大爬坡倾角", &m_agentMaxSlope, 0.0f, 90.0f, 1.0f); //Max Slope
	
	/*imguiSeparator();
	imguiLabel("区域设置"); //Region
	imguiSlider("最小区域大小", &m_regionMinSize, 0.0f, 150.0f, 1.0f);  //Min Region Size
	imguiSlider("合并区域大小", &m_regionMergeSize, 0.0f, 150.0f, 1.0f); //Merge Region Size
	if (imguiCheck("Monotore Partitioning", m_monotonePartitioning))
		m_monotonePartitioning = !m_monotonePartitioning;
	
	imguiSeparator();
	imguiLabel("导航网格多边形设置");	//Poligonization
	imguiSlider("最大边长度", &m_edgeMaxLen, 0.0f, 50.0f, 1.0f); //Max Edge Length
	imguiSlider("最大边误差", &m_edgeMaxError, 0.1f, 3.0f, 0.1f);	//Max Edge Error
	imguiSlider("多边形内点的个数", &m_vertsPerPoly, 3.0f, 12.0f, 1.0f); //Verts Per Poly 

	imguiSeparator();
	imguiLabel("网格细节设置");	//Detail Mesh
	imguiSlider("采样距离", &m_detailSampleDist, 0.0f, 16.0f, 1.0f); //Sample Distance
	imguiSlider("最大采样误差", &m_detailSampleMaxError, 0.0f, 16.0f, 1.0f); //Max Sample Error
	*/
	imguiSeparator();
}
