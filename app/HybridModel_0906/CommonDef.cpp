						   ...
						   , Vector2 leftBottom, Vector2 rightTop, Vector2 curPos)
{
	float x = curPos.x();
	float y = curPos.y();
	float x0 = leftBottom.x();
	float y0 = leftBottom.y();
	float x1 = rightTop.x();
	float y1 = rightTop.y();

	float fR0 =	(frb * (x - x0) + flb * (x1 - x)) / (x1 - x0);  // on y = y0
	float fR1 =	(frt * (x - x0) + flt * (x1 - x)) / (x1 - x0);  // on y = y1
	float f = (fR1 * (y - y0) + fR0 * (y1 - y) ) / (y1 - y0);
	return f;
}
...
Vector2 bilateralInterpolation(Vector2 vlb, Vector2 vrb, Vector2 vrt, Vector2 vlt
							, Vector2 leftBottom, Vector2 rightTop, Vector2 curPos)
{
	float fx = bilateralInterpolation(vlb.x(), vrb.x(), vrt.x(), vlt.x(), leftBottom, rightTop, curPos);
	float fy = bilateralInterpolation(vlb.y(), vrb.y(), vrt.y(), vlt.y(), leftBottom, rightTop, curPos);
