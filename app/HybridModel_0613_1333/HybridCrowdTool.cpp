...
void HybridCrowdToolState::addAgent(const float* p, int color)
{
	if (!m_sample) return;
	HybridModelCrowd* crowd = m_sample->getHybridCrowd();
	...
	int idx = crowd->addAgent(p, &ap, NULL, NULL, color);
	if (idx != -1)
	{
		if (m_targetFace >= 0)
...
//			if (dtStatusSucceed(status))
	...
	if (imguiCollapse("选择人群智能体的颜色", 0, params->m_expandAgentColor))//Selected Debug Draw
	...
	imguiSeparatorLine();
	imguiSlider("稠密人群阈值", &m_densityThreshold, 0.05f, 1.1f, 0.05f);
...
void HybridCrowdTool::handleClick(const float* s, const float* p, bool shift)
{
	...
	if (m_mode == TOOLMODE_CREATE)
	{
		...
		{
			// Add
			m_state->addAgent(p, params->m_selectedAgentColor);
		}
	}
