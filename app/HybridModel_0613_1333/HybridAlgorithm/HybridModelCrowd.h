...
public:
	...
	RVO::RVOSimulator* m_groupRVOSim;
...
public:
	...
	bool init(const int maxAgents, const float maxAgentRadius, NavMesh* nav, NavMeshQuery* nmq, const float grid_size, const float ox, const float oy,const float ex, const float ey);
	...
	int addAgent(const float* pos, const dtCrowdAgentParams* params, float* target = NULL, float* vel = NULL, int color = 0);
