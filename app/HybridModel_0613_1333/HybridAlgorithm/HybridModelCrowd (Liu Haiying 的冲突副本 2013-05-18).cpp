...
HybridModelCrowd::HybridModelCrowd():m_agents(0),m_maxAgents(0),m_activeAgents(0),m_pathResult(0),m_navquery(0),m_grids(0),m_grid(0),m_root(0),m_rvosim(0), m_fluidsim(0),m_activeGrids(0),m_maxGrids(0)
{
...
bool HybridModelCrowd::init(const int maxAgents, const float maxAgentRadius, dtNavMesh* nav, const float grid_size, const float ox, const float oy, const float ex, const float ey)
{
	purge();
...
void HybridModelCrowd::updateTopologyOptimization(dtCrowdAgent** agents, const int nagents, const float dt)
{
	if (!nagents)
		return;
...
*/
	...
	for(int i=0; i<m_maxGrids;i++)
	{
		...
		if(m_activeGrids[i]->now_density >= Grid::HighDensity)
...
*/
	...
	double simulatetime = (double)GetTickCount() - st;
	printf("simulatetime:%.20lf\n",simulatetime);
...
*/	
	double totaltime = (double)GetTickCount() - tt;
	printf("totaltime:%.20lf\n",totaltime);
}
