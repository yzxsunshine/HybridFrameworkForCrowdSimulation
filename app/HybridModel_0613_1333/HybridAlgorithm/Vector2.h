...
inline void SetX(const Vector2& vector)
{
	x_ = vector.x();
}
...
inline void SetY(const Vector2& vector)
{
	y_ = vector.y();
}
...
inline double getDistance(const Vector2& vec)
{
	return sqrt((x_-vec.x())*(x_-vec.x())+(y_-vec.y())*(y_-vec.y()));
}
