...
bool HybridModelCrowd::init(const int maxAgents, const float maxAgentRadius, NavMesh* nav, NavMeshQuery* nmq, const float grid_size, const float ox, const float oy, const float ex, const float ey)
{
	...
	m_groupRVOSim->setAgentDefaults(grid_size/2, PI*grid_size*grid_size/8/maxAgentRadius, 5.0f, 5.0f, maxAgentRadius, 2.0f/*max speed*/);
...
int HybridModelCrowd::addAgent(const float* pos, const dtCrowdAgentParams* params, float* target, float* vel, int color)
{
	// Find empty slot.
	int idx = -1;
	for (int i = 0; i < m_maxAgents; ++i)
...
#pragma omp parallel for
	...
	for(int i=0; i<m_fluidgroups.size(); i++)
	{
		...
		for(int j=0; j<m_fluidgroups[i].contours.size(); j++ )
		{
			std::vector<Vector2> obstacle = m_fluidgroups[i].contours[j];
			for(int k=0; k<obstacle.size(); k++)
				obstacle[k] = obstacle[k] + m_fluidgroups[i].avgVel*dt*300.0f;
...
void HybridModelCrowd::MergeGrid()
{
	...
	{
		if((*m_grids)[i].m_densityStatus == GRAY)
		{
		...
		if((*m_grids)[i].m_densityStatus==WHITE)
			continue;
		if((*m_grids)[i].m_densityStatus==BLACK)
		{
...
void HybridModelCrowd::save(FILE* fp)
{
	...
	{
		HmAgent* ag = agents[i];
		fprintf(fp, "%f %f %f %f %f %f %d\n", ag->npos[0], ag->npos[1], ag->npos[2], ag->targetPos[0], ag->targetPos[1], ag->targetPos[2], ag->color_id);
	}
}