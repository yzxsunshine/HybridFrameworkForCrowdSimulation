...
void Tile::BuildGrids(NavMesh* nav, std::vector<Grid>& grids)
{
	...
	for(int i=1; i<faceNum; i++)
	{
		...
		for (unsigned int j = 0; j<gridCRId.size(); j++)
		{
...
void Tile::BuildGridNeighbor(NavMesh* nav, std::vector<Grid>& grids)
{
	...
	{
		int gid = m_gridIds[i];
		for (unsigned int j = 0; j<grids[gid].m_faces.size(); j++)
		{
	...
	for(int l=0; l<gridNum; l++)
	{
		...
		for(int i=0; i<4; i++)	// traverse all neighbor of the tile
		{
			...
			if(m_neighbor[i]!=NULL)
			{
				...
				for (unsigned int j = 0; j<m_neighbor[i]->m_gridIds.size() && !foundNeighbor; j++)	// traverse all grids of the neighbors
				{
					int ngid = m_neighbor[i]->m_gridIds[j];
					for (unsigned int k = 0; k<grids[ngid].m_faces.size() && !foundNeighbor; k++)	// traverse all faces of the grid to find if any faces are adjacent to current grid
					{
...
bool Tile::GetNearestFace(const float* pos, int& gridID, int& faceID, NavMesh* nav, std::vector<Grid>& grids, float* nearestPos)
{
	...
	for(unsigned int i=0; i<m_gridIds.size(); i++)
	{
		int gid = m_gridIds[i];
