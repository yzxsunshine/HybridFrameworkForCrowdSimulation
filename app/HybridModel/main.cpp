...
struct RegionPoint	//used for multi agents selection
{
	float x;
	float y;
	float rays[3];
...
HybridModel* createHybridModel() { return new HybridModel(); };
...
//#endif
	...
	char sampleName[64] = "Choose Simulation Tools"; 
	
	FileList files;
	char meshName[128] = "Choose Input Mesh";

	FileList testFiles;
	char testName[128] = "Choose Test Case";
	...
	while(!done)
	{
		...
		if (showMenu)
		{
			const char msg[] = "W/S/A/D: move, RMB: rotate camera";
			imguiDrawText(280, height-20, IMGUI_ALIGN_LEFT, msg, imguiRGBA(255,255,255,128));
		}
		
		if (showMenu)
		{
			if (imguiBeginScrollArea("Scene Property", width-250-10, 10, 250, height-20, &propScroll))
				mouseOverMenu = true;
			...
			imguiSeparator();
			imguiLabel("Input Mesh"); //Input Mesh
		...
		if (showLevels)
		{
			...
			if (levelToLoad != -1)
			{
				...
				if (!mesh || !mesh->ReadMap(path))
				{
					...
					showLog = true;
					logScroll = 0;
					ctx.dumpLog("Geom load log %s:", path);//
				}
				else
				{
		...
		if (showTests)
		{
			...
			if (levelToLoad != -1)
			{
				...
				while(!feof(fpTest))
				{
					...
					fscanf(fpTest, "%f %f %f %f %f", &simParams.m_maxNeighborDist, &simParams.m_maxNumNeighbors
												   , &simParams.m_planHorizon, &simParams.m_agentRadius,  &simParams.m_preferSpeed);
					
					hmc->addAgent(p, &ap, simParams, t, NULL, colorID);
				}
				fclose(fpTest);
			}
			imguiEndScrollArea();
		...
		if (showLog && showMenu)
		{
			if (imguiBeginScrollArea("Log", 250+20, 10, width - 300 - 250, 200, &logScroll)) //Log
				mouseOverMenu = true;
		...
		{
			if (imguiBeginScrollArea("Tools", 10, 10, 250, height-20, &toolsScroll)) //Tools
				mouseOverMenu = true;
