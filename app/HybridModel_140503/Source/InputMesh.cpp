		...
		for(int j=0; j<3; j++) // traverse neighbor faces
		{
			...
			for(int k=0; k<3; k++)	// traverse vertexes of current face
			{
				for(int l=0; l<3; l++)	//traverse vertexes of neighbor face
				{
					...
					{
						m_navMesh.edges[edgeNum].vertIDs[edgeVertNum] = m_navMesh.face[fid].vertexIDs[k];
						m_navMesh.edges[edgeNum].vertRelIDs[edgeVertNum] = k;
						m_navMesh.vert[m_navMesh.face[fid].vertexIDs[k]].edgeIDs.push_back(edgeNum);
						edgeVertNum++;
					}
				}
			}
...
void InputMesh::BuildNavMesh()
{
	...
	node.AStarSearch(m_navMesh.face[9], m_navMesh.face[4]
				   , m_midPtCtrl.m_midPointList[18].position, m_midPtCtrl.m_midPointList[10].position
				   , m_midPtCtrl.m_midPointList, path, wayPoints);
	std::vector<vcg::Point3f> smoothedPath = node.AStarPathSmooth(m_navMesh, path, wayPoints);*/
}
