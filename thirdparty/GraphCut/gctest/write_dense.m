...
maxx = 87.805496
maxz = 77.079002
[X, Y] = meshgrid(0:2:60, 0:2:60);
X2 = X + (maxx-70)*ones(size(X, 1), size(X,2));
Y2 = Y + (minz+10)*ones(size(Y, 1), size(Y, 2));
X1 = X + (minx+10)*ones(size(X, 1), size(X, 2));
Y1 = Y + (minz+10)*ones(size(Y, 1), size(Y, 2));
X3 = X + (maxx-70)*ones(size(X, 1), size(X, 2));
Y3 = Y + (maxz-70)*ones(size(Y, 1), size(Y, 2));
X4 = X + (minx+10)*ones(size(X, 1), size(X, 2));
Y4 = Y + (maxz-70)*ones(size(Y, 1), size(Y, 2));

fid = fopen('dense_5000.txt', 'w');
